...
export class DataBaseService {
    ...
    async presentToast(msj : string) {
        const toast = await this.toastController.create({message: msj, duration: 3000, icon: 'globe'});

        await toast.present();
    }
    async presentAlert(msj : string) {
        const alert = await this.alertController.create({header: 'Alert', message: msj, buttons: ['OK']});

        await alert.present();
    }
    async createTables() {
        try { // executing sql querys
    ...
    createDB() { // verificamos que la plataforma este lista
        this.platform.ready().then(() => { // creamos la BD
            this.sqlite.create({name: 'bdasistoqr.db', location: 'default'}).then((db : SQLiteObject) => {
                ...
                this.createTables();
            }).catch(e => { // muestro el mensaje de error en caso de ocurrir alguno
    ...
    searchSubjects() { // retorno la ejecución del select
        return this.database.executeSql('SELECT * FROM asignatura  ', []).then(res => { // creo mi lista de objetos de noticias vacio
            let items: Asignaturas[] = [];
            // si cuento mas de 0 filas en el resultSet entonces agrego los registros al items
            if (res.rows.length > 0) {
                for (var i = 0; i < res.rows.length; i++) {
                    items.push({id: res.rows.item(i).id_asignatura, nombre: res.rows.item(i).nombre, sigla: res.rows.item(i).sigla})
                }
    ...
    searchSubjectsU(idu) { // retorno la ejecución del select
        return this.database.executeSql('SELECT *  FROM asig_secc JOIN listado  using(id_asig_secc)  JOIN asignatura  USING(id_asignatura) JOIN seccion USING(id_seccion) WHERE id_usuario = (?)', [idu]).then(res => { // creo mi lista de objetos de noticias vacio
            let items: AsigStudent[] = [];
            // si cuento mas de 0 filas en el resultSet entonces agrego los registros al items
            if (res.rows.length > 0) {
                for (var i = 0; i < res.rows.length; i++) {
                    items.push({id_usuario: res.rows.item(i).id_usuario, nombre_asig: res.rows.item(i).nombre_asignatura, sigla_asig: res.rows.item(i).sigla_asig, sigla_secc: res.rows.item(i).sigla_secc})
                }
    ...
    searchSections() {
        return this.database.executeSql('SELECT * FROM seccion', []).then(res => {
            ...
            if (res.rows.length > 0) {
                for (var i = 0; i < res.rows.length; i++) {
                    items.push({id: res.rows.item(i).id_asignatura, sigla: res.rows.item(i).sigla})
                }
    ...
    searchSubSect() { // retorno la ejecución del select
        return this.database.executeSql('SELECT * FROM asig_secc JOIN asignatura  USING(id_asignatura) JOIN seccion USING(id_seccion) ', []).then(res => { // creo mi lista de objetos de noticias vacio
            let items: AsigSecc[] = [];
            // si cuento mas de 0 filas en el resultSet entonces agrego los registros al items
            if (res.rows.length > 0) {
                for (var i = 0; i < res.rows.length; i++) {
    ...
    searchAtendance(id) {
        return this.database.executeSql("SELECT * FROM detalle_asist JOIN usuario USING(id_usuario) WHERE id_asistencia = (?)", [id]).then(res => {
            ...
            if (res.rows.length > 0) {
                for (var i = 0; i < res.rows.length; i++) {
                    items.push({nombre: res.rows.item(i).nombre, nombre_usuario: res.rows.item(i).nombre_usuario, apellido: res.rows.item(i).apellido, estado_asistencia: res.rows.item(i).estado_asistencia})
                }
            }
